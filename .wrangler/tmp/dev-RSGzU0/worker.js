...
// .wrangler/tmp/bundle-4eAzoV/checked-fetch.js
...
// .wrangler/tmp/bundle-4eAzoV/strip-cf-connecting-ip-header.js
  ...
  try {
    ...
    try {
      ...
      if (results && results.length > 0) {
        const bookingConfig = JSON.parse(results[0].value);
...
Fampam Team`);
  ...
  try {
    ...
    if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && ADMIN_WHATSAPP_NUMBER) {
      const fromNumber = TWILIO_FROM_NUMBER || "whatsapp:+14155238886";
...
app.post("/stripe/create-checkout-session", async (c) => {
  ...
  const orderNumber = results[0].last_number;
  await c.env.DB.prepare(`
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    customer_email: body.guest_email || void 0
  });
...
app.get("/orders/my", async (c) => {
  ...
  try {
    const payload = await verify2(token, c.env.JWT_SECRET || "secret");
...
app.post("/admin/reservations/:id/reply", async (c) => {
  ...
  try {
    ...
    if (reservation && reservation.email && c.env.RESEND_API_KEY) {
      ...
      bodyText = bodyText.replace(/{name}/g, reservation.name || "").replace(/{party_size}/g, String(reservation.party_size || "")).replace(/{date}/g, reservation.date || "").replace(/{time}/g, reservation.time || "");
      const resendRes = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${c.env.RESEND_API_KEY}`,
...
app.post("/admin/upload", async (c) => {
  ...
  return c.json({ url: `/assets/${filename}` });
});
var rootApp = new Hono2();
rootApp.route("/", app);
rootApp.get("*", async (c) => {
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260526.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260526.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-4eAzoV/middleware-insertion-facade.js
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260526.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-4eAzoV/middleware-loader.entry.ts
